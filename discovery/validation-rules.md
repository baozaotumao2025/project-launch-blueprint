...
| Coverage | `analysis inventory` / `brief` / `story map` / `page` / `feature` / `gwt` / `relations` | inventory 里的每个文件都被显式对账，核心内容都被能力地图接住 | 有文件未对账，或有核心内容漏掉 |
...
2. 负向测试：主动寻找幻觉、重名、页面化、技术化、跨阶段混写、漏项、未对账文件和同词异义。
...
- 不能明确说明回退点
- 反例攻击一击即破
- inventory 中存在未覆盖或未显式排除的文件

## 8. File-Level Accounting Rules

- 每个 `analysis_inventory` 项都必须在 `analysis_coverage_matrix` 中出现一次。
- `analysis_coverage_matrix.status` 只能是 `mapped`、`excluded`、`needs_review` 之一。
- `excluded` 和 `needs_review` 的文件必须进入 `unmapped_files`，并写清原因。
- 如果任何 inventory 文件既不在 coverage matrix，也不在 unmapped_files，就直接失败。
