...
## 1.1 File-Level Accounting

- 每个 `domain_input_inventory` 项都必须在 `domain_coverage_matrix` 中出现一次。
- `domain_coverage_matrix.status` 只能是 `mapped`、`excluded`、`needs_review` 之一。
- `excluded` 和 `needs_review` 的输入必须进入 `unmapped_inputs`，并写清原因。
- 如果任何 inventory 输入既不在 coverage matrix，也不在 `unmapped_inputs`，就直接失败。
